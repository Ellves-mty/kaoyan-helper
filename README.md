...
当前版本：**v1.8** — 文本/图片/PDF 解题 + 英语整篇解析 + 公式渲染 + 学习趋势统计 + 薄弱点出题练习 + GitHub 跨设备同步（gzip 压缩 + Gist/私有仓库双存储 + 删除同步）全流程可用，已部署上线。

- **统计可视化**：按科目查看知识点做题柱状图（次数 + 正确率，<50% 红色标为薄弱点）、薄弱点排行、综合正确率
- **学习趋势**：按天（14 天）/按周（12 周）查看做题量与正确率变化（柱状 + 折线图）
- **薄弱点出题练习**：薄弱点排行一键「出题」→ AI 按考研真题风格生成针对性练习 → 跳转解题并自动计入该知识点统计
